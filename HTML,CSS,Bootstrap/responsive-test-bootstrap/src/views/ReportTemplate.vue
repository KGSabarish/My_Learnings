<template>
  ...
  <div class="container-fluid mt-3">
    <div class="row">
      <div class="report col">
        ...
        <div class="report-filter-bar row  mb-3 row-cols-xxl-6 row-cols-md-3">
          ...
          <div class="col-2 btn-group divalignleft">
          ...
          <!-- Sort Block -->
          <div class="col-2 sort divalignleft">
            <div class="row">
              <div class="col-1">
              ...
              <div class="col-6 sortbydd">
                <div class="btn-group">
                  <button type="button" class="sortbtn dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                    {{ sorByDropdown }}
                  </button>
                  <ul class="dropdown-menu">
                    <li v-for="(column, index) in columns.sort" :key="index">
                      <a class="dropdown-item" href="#" @click="handleSortByChange(column.name, column.label)">{{
                        column.label }}</a>
                    </li>
                  </ul>
                </div>
          ...
          <!-- Search Block -->
          <div class="col-2 srch srchalign">
        ...
        <div v-else class="table-responsive">
          <table class="table table-hover table-borderless custom-table">
            <thead>
              <tr>
                <th v-for="(header, name) in selectedConfigs" :key="name" scope="col"
                  class="col align-middle table-header-sticky">
                  {{ header.label }}
                </th>

                <th scope="col" class="col align-middle cus-table-header-sticky">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody class="table-group-divider">
              <tr v-if="noRecordFound">
                <td class="h3" colspan="5">No Records Found..!</td>
              </tr>
              <tr v-else v-for="transaction in reportTransactions.transactions" :key="transaction.requestId">
                ...
                <td class="sticky">
                  <div class="row">
                    <div class="col d-flex flex-column align-items-center sticky-column"
                      ...
                      <button type="button" class="btn btn-lg btn-download" @click="
                        download(
                          setDownloadData(
                        ...
                        " :disabled="transaction.status != 'ok'"></button>
                      <span>Download</span>
                    </div>
                    <div class="col d-flex flex-column align-items-center" v-if="tableProps.actions.diff">
                      <button type="button" class="btn btn-lg btn-show-difference" @click="
                        setValuesToStore(
                          'showDifference',
                          reportTransactions.userId,
                          transaction.requestId,
                    ...
                    <div class="col d-flex flex-column align-items-center" v-if="tableProps.actions.transform">
                      <button type="button" class="btn btn-lg btn-transform" @click="
                        setValuesToStore(
                          'xmlTransform',
                          reportTransactions.userId,
                          transaction.requestId,
                    ...
                    <div class="col d-flex flex-column align-items-center" v-if="tableProps.actions.delete">
                      <button type="button" class="btn btn-lg btn-delete" @click="
                        confirmDelete(
                          reportTransactions.userId,
                    ...
                    <div class="col d-flex flex-column align-items-center" v-if="tableProps.actions.restore">
                      <button type="button" class="btn btn-lg btn-restore" @click="
                        restore(
                          reportTransactions.userId,
                          tableProps.type,
                          transaction.requestId,
    ...
    <div class="col d-flex flex-column align-items-center" v-if="$parent.$options.name != 'TrashView'">
      ...
      <button type="button" class="btn btn-lg btn-trash"
        @click="setValuesToStore('trashView', reportTransactions.userId)"></button>
      <span>Trash</span>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async restore(userId, type, requestId,fileName) {
      ...
      try {
        ...
        response = await axios.post("/dev/checkfile", body);
        ...
        if (response.status == 200) {
          ...
          if (isDeleteFile === false) {
            ...
            this.restoreRecord(userId, type, requestId)
          }
        }
      } catch (error) {
    ...
    async restoreRecord(userId, type, requestId) {
      const body = {
        userId,
        type,
        requestId: [requestId],
      ...
      try {
        ...
        const response = await axios.post("/dev/restore", body);
        if (response.status === 200) {
          // this.search();
